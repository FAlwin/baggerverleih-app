...
(function () {
  ...
  window.Screens.kalender = function Kalender({ nav, params = {}, mobile, onMenu, PageHeader }) {
    ...
    // Nur Hauptgeräte (Maschinen + Transport) — Anbaugeräte (kat 'Anbau') ausgeschlossen
    const machines = useMemo(() => store.db.flotte.filter((g) => g.kat === 'Maschine' || g.kat === 'Transport'), [store.db.flotte]);
                              ...
                              return (
                                <div key={di} onClick={() => setDaySheet(day)} style={{ borderLeft: di > 0 ? '1px solid var(--paper-3)' : 'none', background: isWeekend(day) ? 'var(--paper-2)' : 'transparent', cursor: 'pointer', position: 'relative' }}>
                                  <div style={{ display: 'flex', justifyContent: 'center', paddingTop: 5 }}>
                                    <div className="mono" style={{ width: 28, height: 28, borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', background: isToday ? 'var(--ink)' : 'transparent', fontWeight: isToday ? 700 : 600, fontSize: 15, color: isToday ? '#fff' : day < store.today ? 'var(--muted-2)' : 'var(--ink)' }}>{parseInt(day.slice(8), 10)}</div>
                                  </div>
